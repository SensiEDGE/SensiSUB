...
{
  static const uint32_t max_u32 = -1;
  static uint8_t timer_expired = 0;
  
    if (state_machine.is_running) {
      uint32_t current_tick = HAL_GetTick();
      
      if((max_u32 - state_machine.start_time) > state_machine.interval) {
        // Timer expired without overflow
        if((state_machine.start_time + state_machine.interval) < current_tick) {
          timer_expired = 1;
        }
      } else {
        // Timer expired with overflow
        if((state_machine.interval - ((max_u32 - state_machine.start_time))) < current_tick){
          timer_expired = 1;
        }
      }
...
void StateMachine_Process(pwm_buzzer_state_t *sm)
{
    if (sm->index >= sm->length) {
        ...
        sm->start_time = HAL_GetTick();
        sm->interval = sm->plist[sm->index].Interval;
